 ...
 * CharacterRenderer
 * =================
 * Слоевый SVG-рендер персонажа.
 *
 * Порядок слоёв (bottom to top):
 *   shadow → legs → arms → torso → torso_accessory
 *   → weapon_main → weapon_secondary → neck → head → hair → head_accessory
 *
 * Изменения vs предыдущей версии:
 *   - Руки вырендерятся ДО оружия/щита — они всегда позади
 *   - Без причёски (hair-слот пуст) — персонаж лысый
 *   - Без верхней одежды (torso-слот пуст) — обнажённый торс (цвет кожи)
 */
...
const CLASS_PALETTE: Record<string, { armor: string; weapon: string; accent: string }> = {
  ...
  engineer: { armor: '#7a5c38', weapon: '#c0a060', accent: '#4cc9f0' },
...
function resolveTorsoStyle(itemName: string | undefined, rarity: string | undefined): number {
  if (!itemName) return -1 // -1 → обнажённый торс
  const n = itemName.toLowerCase()
...
export function CharacterRenderer({
  ...
  // torsoStyle === -1 → нет одежды, рисуем кожу
  const hasArmor   = torsoStyle >= 0

  const slotGlow = (slot: string) => hoveredSlot === slot ? 'url(#glow)' : 'none'

  const armorColor  = pal.armor
  ...
  return (
    <div className={`${s.wrap} ${className ?? ''}`}>
      ...
      >
        ...
        {/* ── Руки (рендерятся ДО оружия — всегда позади него) ── */}
        ...
        >
          {torsoStyle === -1 ? (
            // Обнажённый торс
            <>
              <rect x="34" y="58" width="32" height="32" rx="6" fill={skinColor} />
              {/* Простые детали тела: ключицы */}
              <line x1="50" y1="60" x2="50" y2="88" stroke={`${skinColor}60`} strokeWidth="0.5" />
              <ellipse cx="44" cy="66" rx="3" ry="2" fill={`${skinColor}80`} />
              <ellipse cx="56" cy="66" rx="3" ry="2" fill={`${skinColor}80`} />
            </>
          ) : (
            // Одежда
            <>
        ...
        {hasCloak && (
          <g
            ...
            style={{ cursor: 'pointer' }}
          >
            <path d="M34 65 Q20 80 22 115 L34 110 Z"
              fill={`${accentColor}60`} stroke={accentColor} strokeWidth="0.5" />
            <path d="M66 65 Q80 80 78 115 L66 110 Z"
              fill={`${accentColor}60`} stroke={accentColor} strokeWidth="0.5" />
          </g>
        )}

        {/* ── Основное оружие (руки всегда позади него) ── */}
        <g
        ...
        >
          {slug === 'mage' ? (
            ...
            <g>
              <rect x="17" y="45" width="4" height="55" rx="2" fill="url(#metalGrad)" />
        ...
        {/* ── Щит / доп. оружие (руки всегда позади него) ── */}
        {hasShield && (
          <g
          ...
          {hasHelmet && (
            <>
              ...
              <rect x="40" y="30" width="20" height="8" rx="3"
                fill={`${armorColor}dd`} stroke={accentColor} strokeWidth="0.8" />
            </>
          )}
        </g>

        {/* ── Волосы: hairStyle === -1 → лысый (ничего не рисуем) ── */}
        {!hasHelmet && hairStyle >= 0 && (
          <g
          ...
          >
            ...
            {hairStyle === 1 && (
              <>
                <path d="M34 34 Q33 18 50 14 Q67 18 66 34 Q62 22 50 21 Q38 22 34 34Z" fill={hairColor} />
