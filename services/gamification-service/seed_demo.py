...
Создаёт:
  ...
  - 12 тестовых пользователей в auth-сервисе (российские ФИО, bio, avatar, birthday)
  - Назначение manager_id
...
# 12 человек — российские / СНГ ФИО, bio, avatar_url, birthday
...
TEST_USERS = [
    ...
    },
    {
        ...
        "project": "Внутренние инструменты",
        "position": "Инженер по информационной безопасности",
        "role": "employee",
        "xp": 1050,
        "level": 8,
        "coins": 145,
...
async def seed_test_users(db: AsyncSession) -> dict[str, str]:
        ...
        if existing_id:
            ...
            await db.execute(
                ...
                """),
                {
                    ...
                    "birthday": u["birthday"], "full_name": u["full_name"],
                    "username": u["username"],
                }
            )
        else:
    ...
    for u in TEST_USERS:
        ...
        print(f"  ✅ {u['username']:20s} → manager: {mgr_username} ({mgr_id[:8]}...)")
...
async def seed(devuser_id: str) -> None:
    async with async_session() as db:
        ...
        for u in TEST_USERS:
            mgr = f"→ {u['manager_username']}" if u.get("manager_username") else "(менеджер)"
            print(f"   {u['full_name']:30s}  {u['email']:28s}  /  {u['password']}  [{u['position']}]  {mgr}")
