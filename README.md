...
| 🗄️ PostgreSQL схема | ✅ Готово | init.sql, все таблицы, индексы, миграции Alembic |
| 🔴 Redis | ✅ Работает | Брокер для Celery |
| 🚪 API Gateway | ✅ Работает | FastAPI, health check, маршрутизация |
| 🔐 Auth Service | ✅ Полностью готов | JWT, регистрация, логин, refresh, `/me`, birthday |
| 🎮 Gamification Service | ✅ Полностью готов | Квесты, XP, бейджи, лидерборды, Celery |
| 🎨 Frontend (React) | 🚧 В активной разработке | Auth, Layout, Admin Panel, Calendar, Members — готово |
| 📡 Event Service | 🚧 В разработке | Календарь событий: дедлайны, дни рождения, праздники |
...
При первом старте `auth-service` автоматически создаёт тестовых пользователей.
...
PATCH /api/v1/auth/me         # Обновление профиля (включая birthday)
POST  /api/v1/auth/refresh    # Обновление access токена
POST  /api/v1/auth/logout     # Выход из системы
GET   /api/v1/members         # Список участников команды
...
### Этап 2: Frontend *Апрель–Июнь 2026* 🚧
- [x] React 18 + TypeScript приложение
- [x] Страница входа / регистрации
- [x] Дашборд: XP, уровень, прогресс-бар
- [x] Страница квестов (принять / завершить)
- [x] Таблица лидеров
- [x] Профиль пользователя с бейджами и полем birthday
- [x] Admin Panel (управление пользователями и квестами)
- [x] Страница участников команды (Members) с сортировкой по роли
- [x] Страница настроек (SettingsPage) с редактированием профиля
- [x] Календарь событий (дедлайны, дни рождения, праздники)

### Этап 3: Event Service *Июнь 2026* 🚧
- [x] Отображение событий в календаре (calendar page)
