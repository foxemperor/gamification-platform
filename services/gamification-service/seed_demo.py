...
Создаёт:
  ...
  - XP-транзакции для каждого пользователя (история начислений)
  - Coin-транзакции для каждого пользователя
...
# ================================================================
# XP ТРАНЗАКЦИИ
# Создаём историю начислений XP, чтобы endpoint /xp/history
# возвращал реальные данные, а не пустой список.
# ================================================================

# Шаблоны XP-транзакций: (amount, source, description)
...
# Сколько транзакций для каждого пользователя (по username)
...
async def seed_xp_transactions(
    ...
    """Создаёт XP-транзакции для каждого пользователя (если их ещё нет)."""

    async def _create_for(uid: str, count: int) -> None:
        # Проверяем: уже есть транзакции?
...
async def seed_coin_transactions(
    ...
    """Создаёт Coin-транзакции для каждого пользователя (если их ещё нет)."""
...
# ================================================================
# СНАПШОТ ЛИДЕРБОРДА
# Заполняем таблицу leaderboard_snapshots чтобы эндпоинт
# GET /leaderboard сразу возвращал данные после seed.
# ================================================================
...
async def seed_leaderboard(
    ...
    """Создаёт снапшот лидерборда для периода 'all_time'."""

    PERIOD = "all_time"

    # Проверяем: снапшот уже существует?
    existing = await db.scalar(
    ...
    # Собираем всех участников: devuser + тестовые пользователи
    entries = []
    ...
    for u in TEST_USERS:
        ...
        if not uid:
            continue
        quests_c = XP_TX_COUNT_PER_USER.get(u["username"], 1)
        badges_c = 1 if u["username"] in ("bob_backend", "dan_devops") else 1
        entries.append({
            "user_id": uid,
            "username": u["username"],
            "full_name": u["full_name"],
            ...
            "level": u["level"],
            "total_coins": u["coins"],
            "quests_completed": quests_c,
            "badges_count": badges_c,
        })

    # Сортируем по XP и проставляем rank
    ...
    for rank, entry in enumerate(entries, start=1):
        ...
        print(f"  ✅ Лидерборд #{rank}: {entry['username']} — {entry['total_xp']} XP (lvl {entry['level']})")
...
async def seed(devuser_id: str) -> None:
    async with async_session() as db:
        ...
        print("\n📌 Шаг 5/7: Назначаем квесты / бейджи...")
